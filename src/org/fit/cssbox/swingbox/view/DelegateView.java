...
public class DelegateView extends CompositeView {
	...
	if (offset < offset + length && views.length > 0) {
	    /*Actually, we remove old view only, if we have some view to add.
	     *We can not stay empty (no child, no LeafElement-View), otherwise
	     *run into troubles...
	     */
	    if (view != null) {
		view.setParent(null);
	...
	if (views.length > 0) {
	    ...
	    view = tmp;
	    /*setParent is guaranteed to be first method called
	...
	if (data instanceof DelegateElement) {
	    return ((DelegateElement)data).getDelegateName();
	}
	return null;
    }
