...
/**
 * The Class BrowserPane - JEditorPane based component capable to
 * render HTML + CSS. This is alternative to HTMLEditorKit.
 *
 * @author Peter Bielik
 * @version 1.0
 * @since 1.0 - 28.9.2010
...
public class BrowserPane extends JEditorPane{
	...
	return (GeneralEventListener[])listenerList.getListeners(GeneralEventListener.class);
    }
    ...
    /**
     * Renders current content to graphic context, which is returned.
     * May return null;
     *
    ...
    /**
     *  Renders current content to given graphic context, which is updated and returned.
     *  Context must have set the clip, otherwise NullPointerException is throwen.
     *
     * @param g
	...
	if (g.getClip() == null) throw new NullPointerException("Clip is not set on graphics context");
	ViewFactory factory = getEditorKit().getViewFactory();
	if (factory instanceof SwingBoxViewFactory) {
	...
	registerEditorKitForContentType("application/xhtml+xml",HtmlEditorKitClass,loader);
	//this is not official MIME, but may be used
	registerEditorKitForContentType("text/xhtml",HtmlEditorKitClass,loader);
    }
	...
	try {
		...
		SwingUtilities.invokeLater(new Runnable() {
		    @Override
		    public void run() {
			scrollRectToVisible(new Rectangle(0,0,1,1)); //XXX test_encoding-iso8859-2.html
		    }
		});
	...
	if (type != null) {
	    ...
	    setContentType(type); // >> XXX putClientProperty("charset", charset); !!!
...
//	}
	...
	//Opera 11.50 : Opera/9.80 (X11; Linux i686; U; sk) Presto/2.9.168 Version/11.50
	//CSSBox      : Mozilla/5.0 (compatible; BoxBrowserTest/2.x; Linux) CSSBox/2.x (like Gecko)
	//FireFox     : Mozilla/5.0 (Windows; U; Windows NT 6.0; en-US; rv:1.9b5) Gecko/2008032620 Firefox/3.0b5
	//IE8         : Mozilla/4.0 (compatible; MSIE 8.0; Windows NT 5.1; Trident/4.0; .NET CLR 2.0.50727; .NET CLR 1.1.4322; .NET CLR 3.0.04506.30; .NET CLR 3.0.04506.648)
	//SwingBox    : Mozilla/5.0 (compatible; SwingBox/1.x; Linux; U) CSSBox/2.x (like Gecko)
	/*
	 * An unofficial format, based on the above, used by Web browsers is as follows:
	 * Mozilla/[version] ([system and browser information])
	 ...
	 */
        ...
        conn.setRequestProperty("User-Agent", "Mozilla/5.0 (compatible; SwingBox/1.x; Linux; U) CSSBox/2.x (like Gecko)");
