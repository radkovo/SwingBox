...
/**
 * This class is used by editor kit to convert input data to elements used in document.
 * @author Peter Bielik
 * @version 1.0
 * @since 1.0 - 29.1.2011
 */
public class ContentReader {
     ...
     */
    public List<ElementSpec> read(org.xml.sax.InputSource is, URL url, CSSBoxAnalyzer cba, Dimension dim, Charset charset) throws IOException {
	//ale ked sa pouzije setText() neviem nic o url, nic sa nenastavuje , moze byt null
	...
	//System.err.print("used Reader and encoding ? " + is.getCharacterStream() + "  ,  ");
	...
	//System.err.println("Root min width : " + root.getMinimalWidth() + " ,normal width : " + root.getWidth() + " ,maximal width : " + root.getMaximalWidth());
	...
	for (int i=box.getStartChild(); i < total; i++) {
	    ...
	    if (tmp instanceof TextBox) { // -- text box
		buildText(elements, (TextBox) tmp);
	    }else if (tmp instanceof InlineReplacedBox) { // -- inline boxes
	    ...
	    }else if (tmp instanceof ListItemBox) { // -- List item
	...
	//when there are no special requirements to build an element, use this one
